...
async function joinLeaderTuan() {
  let res =  await updateTuanIdsCDN('https://raw.githubusercontent.com/FearNoManButGod/jd_scripts/main/AuthorShareCode/jd_dreamFactory_tuan.json');
  $.authorTuanIds = [...(res && res.tuanIds || [])]
  if($.authorTuanIds.length==0){
    await notify.sendNotify(`${$.name}`, `未获取到开团数据`, { url: jxOpenUrl });
  }else{
...
async function tuanActivity() {
    ...
    if (tuanConfig && tuanConfig.ret === 0) {
      ...
      allMessage += `今日剩余开团次数：${surplusOpenTuanNum}次\n`;
      ...
      if (!tuanId && surplusOpenTuanNum > 0) {
        ...
        if (QueryTuanRes && QueryTuanRes.ret === 0) {
          ...
          for (let item of tuanInfo) {
            ...
            allMessage += `开团情况:${realTuanNum}/${tuanNum}\n`;
            if (realTuanNum === tuanNum) {
              for (let user of userInfo) {
                if (user.encryptPin === $.encryptPin) {
                  if (user.receiveElectric && user.receiveElectric > 0) {
                    ...
                    allMessage += `您在${new Date(user.joinTime * 1000).toLocaleString()}开团奖励已经领取成功\n`;
              ...
              allMessage += `此团未达领取团奖励人数：${tuanNum}人\n团ID为:${tuanId}\n请及时替换。`;
            }
          }
        }
      }
          ...
          if (err) {
            ...
            if (safeGet(data)) {
              ...
              if (data['ret'] === 0) {
                ...
                allMessage += `【开团成功】tuanId为 ${data.data['tuanId']}\n请及时替换。`;
            ...
            if (safeGet(data)) {
              ...
              if (data['ret'] === 0) {
                if (isTuanLeader) {
                  ...
                  allMessage += `【开团(团长)奖励】${data.data['electric']}领取成功\n`;
                  ...
                  allMessage += `【参团奖励】${data.data['electric']}领取成功\n`;
                }
...
function requireConfig() {
    return new Promise(async resolve => {
        ...
        if (!tuanActiveId) {
            await updateTuanIdsCDN('https://raw.githubusercontent.com/FearNoManButGod/jd_scripts/main/AuthorShareCode/jd_dreamFactory_tuan.json');
