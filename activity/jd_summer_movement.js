...
!(async () => {
  ...
  console.log(`类似 app.5c2472d1.js、app.c7364f20.js 等都删除\n不用每次删 执行失败再删`);
	...
	let res = await getAuthorShareCode('https://raw.githubusercontent.com/FearNoManButGod/AuthorCode/main/summer_movement.json')
	if (!res) {
		$.http.get({url: 'https://purge.jsdelivr.net/gh/FearNoManButGod/AuthorCode@main/summer_movement.json'}).then((resp) => {}).catch((e) => console.log('刷新CDN异常', e));
		await $.wait(1000)
		res = await getAuthorShareCode('https://cdn.jsdelivr.net/gh/FearNoManButGod/AuthorCode@main/summer_movement.json')
	}
	$.AuthorShareCode = [...(res || [])]
  ...
  for (let i = 0; i < cookiesArr.length; i++) {
	...
	for (let k = 0; k < $.AuthorShareCode.length ; k++) {
      $.inviteId = $.AuthorShareCode[k];
      console.log(`${$.UserName}去助力作者,助力码${$.inviteId}`);
